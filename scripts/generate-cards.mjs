...
const card = (title, body, height, note = 'AUTO-REFRESH: EVERY 6 HOURS') => `<svg xmlns="http://www.w3.org/2000/svg" width="920" height="${height}" viewBox="0 0 920 ${height}" role="img"><defs><linearGradient id="background" x2="1" y2="1"><stop stop-color="#0b0f17"/><stop offset="1" stop-color="#15102a"/></linearGradient><linearGradient id="line" x2="1"><stop stop-color="#8b5cf6" stop-opacity=".12"/><stop offset=".5" stop-color="#c084fc"/><stop offset="1" stop-color="#8b5cf6" stop-opacity=".12"/></linearGradient></defs><rect x="1" y="1" width="918" height="${height - 2}" rx="20" fill="url(#background)" stroke="#8b5cf6" stroke-opacity=".72" stroke-width="2"/><rect x="18" y="18" width="884" height="${height - 36}" rx="13" fill="none" stroke="#fff" stroke-opacity=".055"/><circle cx="48" cy="48" r="6" fill="#ff5f57"/><circle cx="70" cy="48" r="6" fill="#febc2e"/><circle cx="92" cy="48" r="6" fill="#28c840"/><text x="124" y="55" fill="#c4b5fd" font-family="${font}" font-size="20" font-weight="700">&gt; ${escape(title)}</text><rect x="42" y="82" width="836" height="1" fill="url(#line)"/>${body}<rect x="42" y="${height - 54}" width="836" height="1" fill="url(#line)"/><text x="42" y="${height - 29}" fill="#64748b" font-family="${font}" font-size="12">${escape(note)}</text></svg>`;
const metric = (x, name, value) => `<text x="${x}" y="125" fill="#94a3b8" font-family="${font}" font-size="13">${escape(name).toUpperCase()}</text><text x="${x}" y="157" fill="#f5f3ff" font-family="${font}" font-size="25" font-weight="700">${escape(value)}</text>`;
...
async function liveData() {
  const fallback = { profile: { public_repos: 0, followers: 0 }, repos: [], events: [], languages: [['TypeScript', 46], ['JavaScript', 28], ['CSS', 14], ['HTML', 12]], contributions: 'N/A', weeks: Array.from({ length: 24 }, () => Array(7).fill(0)) };
  try {
    ...
    return { profile, repos, events, languages: languages.length ? languages : fallback.languages, contributions, weeks };
...
async function generate() {
  ...
  const level = count => count ? (count > 10 ? '#c084fc' : count > 5 ? '#8b5cf6' : count > 2 ? '#5b3c91' : '#2a1e45') : '#171427';
  const squares = live.weeks.flatMap((week, x) => week.map((count, y) => `<rect x="${42 + x * 31}" y="${202 + y * 25}" width="17" height="17" rx="3" fill="${level(count)}"/>`)).join('');
  const stats = card('github-status', `${metric(42, 'Public repos', live.profile.public_repos)}${metric(254, 'Stars', stars)}${metric(466, 'Followers', live.profile.followers)}${metric(678, 'Contributions', live.contributions)}<text x="42" y="192" fill="#94a3b8" font-family="${font}" font-size="13">CONTRIBUTION ACTIVITY / LAST 24 WEEKS</text>${squares}<text x="42" y="403" fill="#64748b" font-family="${font}" font-size="12">LESS</text><rect x="83" y="393" width="14" height="14" rx="3" fill="#171427"/><rect x="103" y="393" width="14" height="14" rx="3" fill="#2a1e45"/><rect x="123" y="393" width="14" height="14" rx="3" fill="#5b3c91"/><rect x="143" y="393" width="14" height="14" rx="3" fill="#8b5cf6"/><rect x="163" y="393" width="14" height="14" rx="3" fill="#c084fc"/><text x="187" y="404" fill="#64748b" font-family="${font}" font-size="12">MORE</text>`, 465);
  const colors = ['#60a5fa', '#facc15', '#a78bfa', '#fb923c', '#34d399'];
  const languages = card('top-languages', live.languages.map(([name, percent], i) => `<circle cx="55" cy="${130 + i * 49}" r="8" fill="${colors[i]}"/><text x="76" y="${136 + i * 49}" fill="#e9d5ff" font-family="${font}" font-size="17">${escape(name)}</text><text x="850" y="${136 + i * 49}" text-anchor="end" fill="#c4b5fd" font-family="${font}" font-size="17" font-weight="700">${percent}%</text><rect x="76" y="${146 + i * 49}" width="774" height="5" rx="2.5" fill="#251d3c"/><rect x="76" y="${146 + i * 49}" width="${Math.max(8, Math.round(774 * percent / 100))}" height="5" rx="2.5" fill="${colors[i]}"/>`).join(''), 415);
  const activityRows = (live.events.length ? live.events : [{ type: 'PushEvent', repo: { name: user } }]).slice(0, 5).map((event, i) => `<circle cx="53" cy="${130 + i * 48}" r="5" fill="#7ee787"/><text x="73" y="${136 + i * 48}" fill="#e9d5ff" font-family="${font}" font-size="15">${escape(eventText(event))}</text>`).join('');
  const activity = card('recent-activity', activityRows, 430);
